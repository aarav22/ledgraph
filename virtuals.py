    ...
    def thread_function(self):
        while True:
            ...
            if (
                ...
                # if self.DOUNTER < 1:
                #     self.DOUNTER += 1
                #     print(self.assembled_frame)
                #     import json
                #     with open('test.json', 'w') as f:
                #         json.dump(self.assembled_frame.tolist(), f)

                import json
                with open('test.json', 'r') as f:
                    try:
                        frame = np.array(json.load(f))
                        self.assembled_frame = frame
                    except json.decoder.JSONDecodeError:
                        print("Exception", self.assembled_frame)
                        pass
                        # self.assembled_frame = np.zeros((self.pixel_count, 3))
